...
class TaxiColectivoConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if self.chofer_id:
            ...
            self.grupo_personal = (
                f"chofer_{self.chofer_id}"
            )

        else:

            self.grupo_personal = (
                f"cliente_{self.usuario_id}"
            )
    ...
    async def disconnect(
        ...
        # IMPORTANTE:
        #
        # NO marcar al chofer como inactivo aquí.
        #
        # La conexión WebSocket puede cerrarse temporalmente.
        #
        # El estado del turno se controla mediante:
        #
        # /api/v1/choferes/cambiar-modalidad/
        #
        # o mediante el endpoint de ubicación.
    ...
    async def receive(
        ...
        if action == "finalizar_turno":
            ...
            # Seguridad:
            # el chofer solo puede finalizar
            # su propio turno.

            if self.chofer_id != int(
                chofer_id
            ):

                return

            await self.marcar_chofer_inactivo(
                self.chofer_id
            )
    ...
    async def broadcast_chofer_desconectado(
        self,
        event
    ):
    ...
    async def notificar_cliente_colectivo(
        self,
        event
    ):
    ...
    async def notificar_nueva_solicitud_especial(
        self,
        event
    ):
    ...
    async def notificar_viaje_aceptado(
        self,
        event
    ):
    ...
    async def notificar_sin_chofer_disponible(
        self,
        event
    ):
